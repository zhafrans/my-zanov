...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-primary-800">Edit Customer</h2>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form method="POST" action="{{ route('customers.update', $customer->id) }}" class="p-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="mt-1 block w-full border {{ $errors->has('address') ? 'border-red-500' : 'border-gray-300' }} 
                            rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500">{{ old('address', $customer->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-1">
                    ...
                    <select name="city_id" id="city_id" onchange="getSubdistricts(this.value)"
                        ...
                        @if(old('province_id', $customer->province_id))
                    ...
                    <select name="subdistrict_id" id="subdistrict_id" onchange="getVillages(this.value)"
                        ...
                        @if(old('city_id', $customer->city_id))
                            @foreach($subdistricts as $subdistrict)
                                <option value="{{ $subdistrict->id }}" {{ (old('subdistrict_id', $customer->subdistrict_id) == $subdistrict->id ? 'selected' : '') }}>{{ $subdistrict->name }}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="col-span-1">
                    ...
                    <select name="village_id" id="village_id"
                        ...
                        @if(old('subdistrict_id', $customer->subdistrict_id))
                            @foreach($villages as $village)
                                <option value="{{ $village->id }}" {{ (old('village_id', $customer->village_id) == $village->id ? 'selected' : '') }}>{{ $village->name }}</option>
                            @endforeach
                        @endif
                    </select>
                    @error('village_id')
...
// Initialize dropdowns with customer data
document.addEventListener('DOMContentLoaded', function() {
    @if($customer->province_id)
        getCities({{ $customer->province_id }}).then(() => {
            @if($customer->city_id)
                document.getElementById('city_id').value = {{ $customer->city_id }};
                getSubdistricts({{ $customer->city_id }}).then(() => {
                    @if($customer->subdistrict_id)
                        document.getElementById('subdistrict_id').value = {{ $customer->subdistrict_id }};
                        getVillages({{ $customer->subdistrict_id }}).then(() => {
                            @if($customer->village_id)
                                document.getElementById('village_id').value = {{ $customer->village_id }};
                            @endif
                        });
                    @endif
                });
